...
function fish_prompt
  ...
  if test $__pure_fresh_session -eq 1
    pre_prompt
  end
...
# To send a notification after long command ends
function fish_right_prompt
    ...
    set exclude "nano" "vim" "python"

    if string match -q -r (string join "|" $exclude) $command_name
        return
    end
    ...
    if [ $finish_duration -gt $notify_duration ]
        # Display an icon, name of the command and duration/status below it
        notify-send --expire-time=2000 --category=shell --icon=appointment-soon "$command_name" "Finished in: $finish_duration secs\n Status $exit_status"
    end
end
